...
// 无session请求
const nspost = (method, data, fun) => {
    data = data ? data : "";
    sendPost(dApiUrl + method, data, false, fun)
}
const nspost2 = (method, data, fun) => {
    data = data ? data : "";
    sendPost('http://kpbase.shejidedao.cn/interface-server/api/' + method, data, false, fun)
}
// 普通nspste请求 无session
const nspste = (method, data, fun) => {
    data = data ? data : "";
    sendPost(dApiUrl + method, data, false, fun)
}
...
// 微服务post请求
const wPost = (method, data, fun) => {
    sendPost(wApiUrl + method, data, false, fun)
}

// 独立接口post请求
const dPost = (method, data, fun) => {
    data = data ? data : "";
    ...
    common.codeList.reduce((pre, cur) => {
        if (cur.name == method) {
            data.code = cur.id;
        }
    }, {})
    ...
    // data.sessionID = common.getStorage("sessionID")
    // data.siteID = data.siteID ? data.siteID : common.siteID;
    // data.deviceID = common.deviceID;
    // data.applicationID = common.applicationID;
    sendPost(dApiUrl + method, data, false, fun)
}

// 独立接口post请求 安卓
const dPostAndroid = (method, domainName, data, fun) => {
    data = data ? data : "";
    if (common.getStorage("sessionID")) {
...
// axios 发送请求统一处理
const sendPost = async (url, data, config, fun) => {
    data = {
        ...data,
...
export const uploadFile = (data, callback) => {
    ...
    data.timeout = 6000
    // sendPost('https://web.shejidedao.com/interface-server/api/uploadOneFileToQiniu?fileServerID=ff80808180fe339c0180fee3768f0000&fileBucketID=ff80808180fe339c0180fef85d450003', data, true, callback)
    ...
    // sendPost('https://www.haoju.me/interface-server/api/uploadOneFileToQiniu?fileServerID=8a2f462a88b377e10188bd1b2975053c&fileBucketID=8a2f462a88b377e10188bd20a3e5053d', data, true, callback)
    // sendPost('https://web.shejidedao.com/interface-server/api/' + 'uploadQiniu.json', data, true, callback)
    // sendPost('https://kpinterface.shejidedao.cn/interface-server/api/' + 'uploadFile.json', data, true, callback)
}
...
export const uploadFile2 = (data, callback) => {
    // data.append('method', 'images.upload')
    data.timeout = 6000
    // sendPost('https://web.shejidedao.com/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
    sendPost('http://kpbase.bonjourmed.com/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
    // sendPost('https://www.haoju.me/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
    // sendPost('https://kpinterface.shejidedao.cn/interface-server/api/' + 'uploadFile.json', data, true, callback)
}
...
export const uploadFileProposal = (data, callback) => {
    data.append('method', 'images.upload')
    data.timeout = 6000
    // sendPost('https://web.shejidedao.com/interface-server/api/' + 'uploadFile.json', data, true, callback)
    // sendPost('https://web.shejidedao.com/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
    sendPost('http://kpbase.bonjourmed.com/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
    // sendPost('https://www.haoju.me/interface-server/api/' + 'uploadOneFileToQiniu.json', data, true, callback)
